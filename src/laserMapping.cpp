...
void standard_pcl_cbk(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
    ...
    if (msg->header.stamp.toSec() < last_timestamp_lidar) // 判断是否此时刻的时间戳小于上一帧时刻的时间戳
    {
        ROS_ERROR("lidar loop back, clear buffer");
        lidar_buffer.clear();
    }

    PointCloudXYZI::Ptr ptr(new PointCloudXYZI());  // 定义一个点云指针ptr
    p_pre->process(msg, ptr); // 将msg转换成ptr
...
void imu_cbk(const sensor_msgs::Imu::ConstPtr &msg_in)
{
    ...
    sensor_msgs::Imu::Ptr msg(new sensor_msgs::Imu(*msg_in));  // 定义一个新的指针

    if (abs(timediff_lidar_wrt_imu) > 0.1 && time_sync_en)  // 判断timediff_lidar_wrt_imu 雷达和imu之间是否有差值  差值在配置文件
    {
        msg->header.stamp =
    ...
    imu_buffer.push_back(msg); // imu_buffer 是队列 
    mtx_buffer.unlock();
    sig_buffer.notify_all();
}
...
int main(int argc, char **argv)
{
    ...
    downSizeFilterSurf.setLeafSize(filter_size_surf_min, filter_size_surf_min, filter_size_surf_min);   // 点云下采样
