...
void Preprocess::process(const sensor_msgs::PointCloud2::ConstPtr &msg, PointCloudXYZI::Ptr &pcl_out)
{
  ...
  *pcl_out = pl_surf;  // fastlio 只有面点 没有交点
}
...
void Preprocess::rs_handler(const sensor_msgs::PointCloud2_<allocator<void>>::ConstPtr &msg)
{
  ...
  /*** These variables only works when no point timestamps given  点云，没有时间戳时用 ***/
  double omega_l = 0.361 * SCAN_RATE; // scan angular velocity
  std::vector<bool> is_first(N_SCANS, true);
  std::vector<double> yaw_fp(N_SCANS, 0.0);   // yaw of first scan point
  std::vector<float> yaw_last(N_SCANS, 0.0);  // yaw of last scan point
  ...
  if (pl_orig.points[plsize - 1].timestamp > 0) // todo check pl_orig.points[plsize - 1].time   // 没有时间戳会=0
  {
    given_offset_time = true;
  ...
  for (int i = 0; i < plsize; i++)
  {
    ...
    added_pt.z = pl_orig.points[i].z;
    added_pt.intensity = pl_orig.points[i].intensity;
    added_pt.curvature = (pl_orig.points[i].timestamp - pl_orig.points[0].timestamp) * 1000.0; // curvature unit: ms 时间戳
    // std::cout << "added_pt.curvature:" << added_pt.curvature << std::endl;
    // 时间戳强制赋予
    if (!given_offset_time)
    {
    ...
    if (i % point_filter_num == 0)  // 粗暴地下采样
    {
      if (added_pt.x * added_pt.x + added_pt.y * added_pt.y + added_pt.z * added_pt.z > (blind * blind) )  // 判断盲区
      {
        pl_surf.points.push_back(added_pt);
      }
    }
